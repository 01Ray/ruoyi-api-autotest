...
所有用例数据来自 api_tests/data/role_query_cases.yaml
...
# 用例数据加载（模块加载时执行一次）
# ============================================================
ROLE_QUERY_CASES = load_cases("role_query_cases.yaml")


@allure.epic("RuoYi 接口自动化")
@allure.feature("角色管理")
@allure.story("数据驱动 - 查询")
class TestRoleQueryDataDriven:
    """角色查询用例 - 全部由 YAML 驱动"""

    @pytest.mark.parametrize(
        "case",
        ROLE_QUERY_CASES,
        ids=[c["title"] for c in ROLE_QUERY_CASES],
    )
    def test_role_query(self, admin_client, case):
        """
        通用查询用例
